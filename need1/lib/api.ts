import type { User, Request, Offer, Thread, Message, FlashDrop } from "../types"
...
export const api = {
  ...
  async getBidsForRequest(requestId: string): Promise<Offer[]> {
    await delay(400)
    return mockOffers.filter((offer) => offer.requestId === requestId)
  },

  async acceptBid(bidId: string): Promise<{ success: boolean }> {
    await delay(500)
    return { success: true }
  },

  async declineBid(bidId: string): Promise<{ success: boolean }> {
    await delay(500)
    return { success: true }
  },

  async getUserServices(userId: string): Promise<Request[]> {
    await delay(400)
    return mockRequests.filter((req) => req.seekerId === userId)
  },
